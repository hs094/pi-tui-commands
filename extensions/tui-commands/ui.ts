...
export function buildItems(
	...
	for (const tool of knownTools) {
		...
		items.push({
			...
			values: ["OFF", "ON"],
			description: avail
				? `${tool.cmd} found on PATH`
				: `"${tool.cmd}" not found on PATH`,
		});
	}
...
export async function showManager(
	...
	const availability = new Map(
		knownTools.map((t) => [t.name, binaryExists(t.name)]),
	);
		...
		return {
			render: (w: number) => {
				...
				lines.push(
					theme.fg(
						"dim",
						"✓ installed  ✗ not found  enter  toggle  /  search  esc  close",
					),
				);
				lines.push(
					theme.fg("dim", "add custom: /tuicmd add <name> [command]"),
				);
				return lines;
			},
